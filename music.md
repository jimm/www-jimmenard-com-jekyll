...
- A big honking seven-foot rack, affectionately called the "Think Tank".
  ...
  chest-high rack of the same name, along with a Commodore SX-64 running my
  own real time MIDI performance software _KeyMaster_.
...
- Line 6 [HX Stomp XL](https://line6.com/hx-stomp-xl/) for the Strat
...
equipment list. You can see it in action in our short demo video
[here](https://www.youtube.com/watch?v=Uf8QA3BE5fU).

- Korg Kronos, sustain pedal, expression pedal used for volume, switch used
  for Leslie simulation speed
- ASM Hydrasynth Deluxe, sustain pedal, expression pedal
...
- MIDI
  ...
  - The iPad is running [Midiflow](https://www.midiflow.com/) and
    ...
    - Midiflow routes the MIDI between the Kronos, Hydrasynth, and Stomp
      when I'm using the USB hub. I also use it to filter out clock messages
      in both directions and and modify the velocity curve when going from
      the Hydrasynth to the Kronos
    - BandHelper sends MIDI program changes to the Kronos, Hydrasynth, and
      Stomp
    - I have the Kronos combinations for each song /also/ send program
      changes to the Hydrasynth and the HX Stomp XL when needed. My normal
      Midiflow setup filters those out, but if the iPad dies and I switch to
      5-pin DIN MIDI they'll get those program changes
  - If the iPad dies, I can use my phone running BandHelper (but no
    Midiflow) or switch to 5-pin DIN MIDI and bypass the USB hub completely
- Optional / Rare
  - Ibanez bass (only for the few gigs where I've had to sub for the bass
    player)
  - Line 6 amp for the bass, though I'll probably not use that anymore and
    use the Stomp instead
...
- [KeyMaster](https://github.com/jimm/juce-keymaster) (the JUCE version) is
  real-time MIDI performance software that I've written. The first
  incarnation was also called [KeyMaster](projects/keymaster/index.html). I
  used that version live from 1986&ndash;1993. It was first written for the
  Commodore 64, later ported to the Atari ST. I half-heartedly started a
  Macintosh version. The next was the BeOS version, then a Ruby version
  ([PatchMaster](https://github.com/jimm/patchmaster/)), next
  [keymaster_objc](http://jimm.github.io/keymaster_objc/), a Mac OS X-only
  simpler version, next a cross-platform GUI version written in C++ using
  wxWidgets ([KeyMaster](https://github.com/jimm/keymaster/wiki)).
...
- [kuticle](https://github.com/jimm/kuticle) is a command-line tool for
  fetching data from a Korg Kronos keyboard via MIDI sysex. It's ideal for
  use by an AI agent skill to answer questions about the Kronos's data (for
  example, "which set list slots use a combi that transmits on channel 2?").
